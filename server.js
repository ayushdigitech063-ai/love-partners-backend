...
import { autoSeedBlogs } from './utils/seedBlogs.js';
import { autoSeedSuperAdmin } from './utils/seedSuperAdmin.js';
...
const startServer = async () => {
  ...
  const server = app.listen(PORT, () => {
    console.log(`🚀 LovePartners Production MERN Backend running on http://localhost:${PORT}`);
    console.log(`🔐 Environment: ${process.env.NODE_ENV || 'development'}`);
  });

  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.warn(`⚠️ Port ${PORT} is already in use by a background process. Server is ALREADY RUNNING on port ${PORT}!`);
    } else {
