...
const autoSeedEscorts = async () => {
  try {
    ...
    if (count === 0) {
      ...
      console.log(
        '🌱 Default escort profiles auto-seeded into MongoDB.'
      );
    } else {
      console.log(
        `ℹ️ Escort profiles already exist (${count}). Skipping auto-seed.`
      );
    }
  } catch (err) {
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      console.log(
        `🚀 LovePartners Production MERN Backend running on port ${PORT}`
      );
